        ...
        <div className='font-sans flex flex-col border border-[#020D1914] p-4 gap-4 rounded-lg overflow-hidden max-w-[400px] hover:shadow-lg hover:border-gray-900 transition-all duration-300'>
            ...
            <div className='flex flex-col gap-2'>
                <div className='flex items-center justify-end'>
                    <p className='text-sm font-bold text-gray-900'>{Number(progress).toFixed(2)}% Funded</p>
                </div>
            ...
            {!admin && !campaigner && <div className='flex items-center justify-center'>
                <Link to={`/home/campaigns/${campaign._id}`} className='bg-gray-900 text-center hover:bg-gray-700 transition-colors duration-200 text-white font-semibold px-4 py-2 rounded-full w-full'>Donate Now</Link>
            </div>}

            {admin && <div className='flex items-center justify-center gap-2'>
                <button onClick={handleDelete} disabled={isPending} className='text-center text-gray-900 disabled:opacity-50 border border-gray-900 transition-colors duration-200 hover:text-gray-700 font-semibold px-4 py-2 rounded-full w-full'>{isPending ? "Deleting..." : "Delete"}</button>
                <Link to={`/admin/campaigns/${campaign._id}`} className='bg-gray-900 text-center hover:bg-gray-700 transition-colors duration-200 text-white font-semibold px-4 py-2 rounded-full w-full'>Preview</Link>
            </div>}

            {campaigner && <div className='flex items-center justify-center gap-2'>
                <button onClick={handleDelete} disabled={isPending} className='text-center text-gray-900 disabled:opacity-50 border border-gray-700 transition-colors duration-200 hover:text-black font-semibold px-4 py-2 rounded-full w-full'>{isPending ? "Deleting..." : "Delete"}</button>
                <Link to={`/user/dashboard/campaigns/${campaign._id}`} className='bg-gray-900 text-center hover:bg-gray-700 transition-colors duration-200 text-black font-semibold px-4 py-2 rounded-full w-full'>Preview</Link>
            </div>}
